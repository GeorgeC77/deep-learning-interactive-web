...
export default function Ch02GenerativeClassifiersPage() {
  return (
    <BishopSectionPage
      ...
      whyCards={[
        {
          question: "为什么生成分类器能检测异常样本？",
          answer: "它建模了完整的输入分布 p(x)，不仅能比较“像哪一类”，还能判断“是否像任何一类”。p(x) 很低的样本自然被识别为异常。",
        },
        {
          question: "为什么共享协方差会得到线性决策边界？",
          answer: "当所有类别的协方差相同时，后验 log-odds 中二次项会抵消，只剩下关于 x 的线性项，因此决策边界是直线。",
        },
      ]}
      counterexamples={[
        "对环状分布的数据强制使用 LDA（共享协方差），得到的线性边界无法分离内外两环——说明强假设会限制模型能力。",
        "在文本分类中，'machine' 和 'learning' 经常共现，朴素贝叶斯的独立性假设会低估这一组合的证据强度——说明条件独立并不总是成立。",
      ]}
            bishopMapping={{
        chapter: "Ch 5",
        section: "5.3",
