...
export default function Ch08GraphicalModelsPage() {
  return (
    <BishopSectionPage
      ...
      whyCards={[
        {
          question: "为什么图模型能简化联合分布？",
          answer: "利用条件独立性，复杂的联合分布可以分解为局部条件概率的乘积，大大减少参数数量。",
        },
        {
          question: "为什么有向边不直接等同于因果关系？",
          answer: "有向边只表示概率依赖，可能是因果、相关或逆因果。判断因果需要额外假设和干预实验。",
        },
      ]}
      counterexamples={[
        "把一阶马尔可夫链的分解套用到一般 DAG，会遗漏某些父节点条件——说明图结构决定因子分解形式。",
        "认为有向边总是表示因果关系，实际上同一组条件独立性可以对应多个不同的因果图——说明因果推断需要额外假设。",
      ]}
            bishopMapping={{
        chapter: "Ch 11",
        section: "11.1",
