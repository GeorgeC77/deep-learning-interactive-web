...
export default function Ch16OverviewPage() {
  return (
    <BishopSectionPage
      ...
      quiz={[
        {
          question: "普通确定性自编码器的主要训练目标是什么？",
          options: [
            "最小化输入与重构之间的差异。",
            "最大化隐变量与标签的互信息。",
            "最小化两个分布之间的 Wasserstein 距离。",
            "最大化后验概率。",
          ],
          correctIndex: 0,
          explanation: "自编码器的核心是最小化重构损失，使解码器能从隐表示恢复输入。",
        },
        {
          question: "为了防止自编码器学到恒等映射，通常可以采取什么措施？",
          options: [
            "限制隐层维度或添加稀疏/去噪/掩码约束。",
            "增加网络层数但不加任何正则化。",
            "把隐层维度设得比输入维度更大。",
          ...
          correctIndex: 0,
          explanation: "欠完备、稀疏、去噪或掩码等约束迫使模型学习数据的有用结构而非简单复制。",
        },
        {
          question: "变分自编码器与普通自编码器的关键区别是什么？",
          options: [
            "编码器输出的是概率分布参数而非单个隐向量。",
            "解码器不需要重构输入。",
            "只能用于离散数据。",
            "不使用反向传播。",
          ],
          correctIndex: 0,
          explanation: "VAE 将编码器视为近似后验 q(z|x)，并通过 ELBO 训练，使解码器成为生成模型。",
        },
      ]}
      bishopMapping={{
        chapter: "Ch 19",
