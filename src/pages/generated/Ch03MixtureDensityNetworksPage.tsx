...
export default function Ch03MixtureDensityNetworksPage() {
  return (
    <BishopSectionPage
      ...
      whyCards={[
        {
          question: "为什么逆问题需要混合密度网络？",
          answer: "普通回归只输出一个平均值，在“同一个输入有多个合理输出”的逆问题中会给出错误折中。MDN 输出完整的条件分布，能同时表示多个可行解。",
        },
        {
          question: "为什么 MDN 的参数要加约束？",
          answer: "混合系数必须非负且和为 1，方差必须为正。不加约束的网络输出可能不是合法的概率分布。",
        },
      ]}
      counterexamples={[
        "对机器人逆运动学用 MSE 回归，预测结果落在两个可行关节角之间的“不可能位置”——说明单值预测在逆问题中必然失败。",
        "把未归一化的核密度当作概率密度，积分不等于 1——说明归一化常数不是可有可无的细节。",
      ]}
            bishopMapping={{
        chapter: "Ch 6",
        section: "6.5",
