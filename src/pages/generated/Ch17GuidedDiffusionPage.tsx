...
export default function Ch17GuidedDiffusionPage() {
  return (
    <BishopSectionPage
      ...
      whyCards={[
        {
          question: "为什么无分类器引导比分类器引导更简单？",
          answer: "分类器引导需要额外训练一个分类器；无分类器引导只需在训练时随机丢弃条件，无需额外模型。",
        },
        {
          question: "为什么 w>1 会降低多样性？",
          answer: "w>1 是向条件方向外推，超出原始条件预测。过度引导会让所有样本都集中在条件模式的中心，忽略数据分布的多样性。",
        },
      ]}
      counterexamples={[
        "用 w=5 引导文本生成图像，结果过饱和且缺乏多样性——说明引导强度需要权衡。",
        "认为 w=1 就是引导——实际上 w=1 只是普通条件采样，0≤w≤1 才是插值，w>1 才是外推。",
      ]}
            bishopMapping={{
        chapter: "Ch 20",
        section: "20.4",
