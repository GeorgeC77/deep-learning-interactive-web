...
export default function Ch16DeterministicAutoencodersPage() {
  return (
    <BishopSectionPage
      ...
      summary={"确定性自编码器通过编码器-解码器结构学习压缩表示。欠完备线性自编码器在平方重构误差下学习与 PCA 相同的主子空间；深层自编码器能学习非线性流形。稀疏、去噪与掩码自编码器通过不同约束获得有意义的特征。"}
      concepts={[
        {
      ...
      learningObjectives={[
        "理解欠完备线性自编码器与 PCA 主子空间的关系。",
        "知道线性自编码器的参数化不唯一，隐空间坐标不一定等于正交 PCA 坐标。",
        "了解深层自编码器相比线性版本的优势。",
        "区分稀疏、去噪与掩码自编码器的约束目标。",
      ]}
      coreIntuition={"自编码器像一台‘压缩-解压机’：好的隐表示不是简单复制输入，而是在压缩过程中保留重建所需的本质结构。线性版本学到的是与 PCA 相同的主子空间，但具体坐标可以有很多种等价选择。"}
      commonMistakes={[
      ...
      quiz={[
        {
          question: "单层线性自编码器（均方误差、恒等激活、隐层维度 M）与 PCA 的关系是什么？",
          options: [
            "它学习与 PCA 相同的主子空间，但参数化不唯一，隐空间坐标不一定等于正交 PCA 坐标。",
            "它与 PCA 完全等价，包括坐标都相同。",
            "它等价于 k-means 聚类。",
            "它只是随机投影。",
          ],
          correctIndex: 0,
          explanation: "欠完备线性自编码器的最优解由数据协方差的前 M 个特征向量张成，即 PCA 主子空间。但 (W, U) 可经任意可逆线性变换重新参数化；只有加正交或 tied-weight 约束时才得到标准 PCA 坐标。",
        },
        {
