  ...
  return (
    <BishopSectionPage
      ...
      whyCards={[
        {
          question: "为什么需要隐变量模型？",
          answer: "高维数据往往由少数几个隐藏因素驱动。显式建模隐变量可以压缩表示、解释结构，并支持生成新样本。",
        },
        {
          question: "为什么 EM 适合隐变量模型？",
          answer: "隐变量未知导致对数似然包含积分/求和，直接求导困难。EM 通过交替推断隐变量和更新参数，把难题分解成两个可解的步骤。",
        },
      ]}
      counterexamples={[
        "认为所有隐变量都是连续的；GMM 的簇标号和 HMM 的状态序列都是离散隐变量——说明隐变量类型取决于任务。",
        "认为因子分析和概率 PCA 完全相同；因子分析允许每个观测维度有独立噪声方差，概率 PCA 假设各向同性噪声——说明细节决定模型能力。",
      ]}
            bishopMapping={{
        chapter: "Ch 16",
        section: "16.2",
