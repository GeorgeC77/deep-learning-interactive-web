...
export default function ROCInteractiveDemo() {
  ...
  const roc = useMemo(
    () => computeROC(data.negScores, data.posScores, numPoints),
    [data],
  );
  ...
  const tpr = current.tpr;
  const fpr = current.fpr;
  ...
  const histBins = useMemo(() => {
    ...
    for (const s of data.negScores) {
      const i = Math.min(bins - 1, Math.floor((s - min) / binW));
      negHist[i]++;
    }
    for (const s of data.posScores) {
      const i = Math.min(bins - 1, Math.floor((s - min) / binW));
      posHist[i]++;
    }
      ...
      <div className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {overlap < 0.05
                ...
                : overlap > 0.95
                  ? '分布高度重叠 → AUC 接近 0.5'
                  : `中等重叠 → 理论 AUC ≈ ${theoreticalAUC(overlap).toFixed(3)}`}
            </p>
          </div>
          <div>
            <div className="flex justify-between text-sm font-medium text-gray-700 mb-1">
            ...
            <Slider
              ...
              max={numPoints}
              step={1}
              onValueChange={(v) => setThresholdIdx(v[0])}
            />
            <p className="text-xs text-gray-500 mt-0.5">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
            ...
            >
              ...
              <line
                x1={toX(
                  (current.threshold - histBins.min) /
                    (histBins.max - histBins.min),
                )}
                y1={M.t}
                x2={toX(
                  (current.threshold - histBins.min) /
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
            ...
            >
              ...
              <text
                ...
                className="text-[10px]"
                fill="#6b7280"
                transform={`rotate(-90,10,${M.t + PH / 2})`}
              >
                TPR（真正例率）
              </text>
              {/* Grid */}
            ...
            <div className="text-center pb-1 text-sm font-bold text-purple-700">
              AUC = {auc.toFixed(3)}（理论 AUC = {theoreticalAUC(overlap).toFixed(3)}）
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-center">
          {[
            ...
            {
              label: '当前阈值',
              value: current.threshold.toFixed(2),
              color: 'text-gray-700',
              bg: 'bg-gray-50',
            },
          ].map((m) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
          {[
            ...
            { label: '精确率', value: (tp + fp > 0 ? tp / (tp + fp) : 0).toFixed(3) },
            { label: '召回率', value: tpr.toFixed(3) },
          ].map((m) => (
            <div
              key={m.label}
              className="rounded-lg p-2 bg-slate-50 border border-slate-100"
            >
              <div className="text-xs text-gray-600">{m.label}</div>
        ...
        <div className="text-sm text-gray-700 space-y-2 bg-slate-50 p-4 rounded-lg border">
          ...
          <ul className="space-y-1 list-disc list-inside">
            <li>
              <strong>降低阈值</strong>（左移竖线）：更多样本被判为正 → TPR↑ 同时 FPR↑ →
              ROC 点沿曲线向右上方移动
            </li>
            <li>
              <strong>升高阈值</strong>（右移竖线）：更保守 → TPR↓ FPR↓ → ROC 点沿曲线向左下方移动
            </li>
            <li>
              <strong>分布式重叠度越小</strong>，ROC 曲线越贴近左上角，AUC 越大
            </li>
            <li>
              <strong>AUC = 1.0</strong>：存在某个阈值完美分离两类；
              <strong>AUC = 0.5</strong>：分类器等同于随机猜测
            </li>
            <li>
              AUC 衡量的不是某一个阈值的好坏，而是分类器在所有可能阈值下的
              <strong>平均排序能力</strong>
            </li>
            <li>
              <strong>AUC = P(score<sub>正例</sub> &gt; score<sub>负例</sub>)</strong>：
              随机抽取一个正例和一个负例，正例分数更高的概率
            </li>
          </ul>
        </div>
      </div>
